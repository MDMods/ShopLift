...
internal static class NoMoreStorePatch
{
    ...
    internal static class PnlStagePatch
    {
        ...
        {
            var tagDifficultyDlcTransform = __instance.transform.FindChild("StageUi").FindChild("Tag&Difficulty&Dlc");
            
            // Hide the store button
            tagDifficultyDlcTransform.FindChild("BtnDlc").gameObject.SetActive(false);

            var bigBarBehindEverythingObj = tagDifficultyDlcTransform.FindChild("ImgAlbumBase").gameObject;
            var tagObj =tagDifficultyDlcTransform.FindChild("BtnOwn").gameObject;
